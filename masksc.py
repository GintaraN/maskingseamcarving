...
DOWNSIZE_WIDTH = 800                      # resized image width if SHOULD_DOWNSIZE is True
...
def create_mask(im):
    ...
    img = cv2.addWeighted(imgori, 0.7, imgcanvas, 0.7, 0)
    ...
    while(1):
        ...
        if k == ord('q'):
            ...
            img[img != 255] = 0  #change non white color to black
            #print (img)
            break
        elif k == 27:
            break
...
if __name__ == '__main__':
    ...
    ap.add_argument("-mask", help="Path to (protective) mask", action='store_true')
    ap.add_argument("-rmask", help="Path to removal mask", action='store_true')
    ...
    mask = create_mask(im) if args['mask'] else None
    rmask = create_mask(im) if args['rmask'] else None
